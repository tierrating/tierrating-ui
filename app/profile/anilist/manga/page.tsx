    ...
    return (
        ...
        >
            <div className="w-full">
                ...
                <p className="truncate text-sm mt-1">{item.title.english || item.title.romaji}</p>
            </div>
        </div>
    );
}
...
function DroppableTier({ tier, items }: { tier: TierConfig; items: RatingItem[] }) {
    ...
    return (
        <div className="flex mb-4">
            ...
            >
                <SortableContext items={tierItems.map(item => item.id.toString())} strategy={horizontalListSortingStrategy}>
...
export default function TierListPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const response = await fetch("http://localhost:8080/data/anilist/manga");
                const data = await response.json();
                // Initialize all items as unassigned
                setItems(data.map((item: RatingItem) => ({ ...item, tier: "unassigned" })));
            } catch (error) {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (active.id !== over.id && !over.id.toString().startsWith('tier-') && over.id !== 'unassigned') {
            setItems((items) => {
                const activeItem = items.find(item => item.id.toString() === active.id.toString());
                const overItem = items.find(item => item.id.toString() === over.id.toString());
                    ...
                    {activeId && activeItem ? (
                        <div className="w-full opacity-80">
                            <AspectRatio ratio={9 / 16}>
                                <Image
                                    fill={true}
                                    src={activeItem.coverImage.large}
                                    alt={activeItem.title.english || activeItem.title.romaji}
                                    className="rounded-md object-cover"
                                />
                            </AspectRatio>
                            <p className="truncate text-sm mt-1">{activeItem.title.english || activeItem.title.romaji}</p>
                        </div>
                    ) : null}
                </DragOverlay>
            </DndContext>
